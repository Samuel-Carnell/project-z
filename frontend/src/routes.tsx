...
const SignIn = () => {
	const signIn = () => {
		...
		queryParam.append('client_id', '48d191339b86000aef45');
		queryParam.append('scope', 'read:user user:email');
		window.location = `https://github.com/login/oauth/authorize?${queryParam}`;
	};
...
const OAuthSignIn = () => {
	...
	useEffect(() => {
		...
		fetch(`${config.apiServer}/sign-in/oauth/${provider}?code=${authorizationCode}`, {
			credentials: 'include',
